...
export const zipMovies =  (zipCode, zipRadius) => {
  return dispatch => {
    ...
    return fetch(`http://data.tmsapi.com/v1.1/movies/showings?startDate=${currentDate}&zip=${zipCode}&radius=${zipRadius}&api_key=${process.env.TMS_API_KEY}`, {
      method: 'GET',
      headers: {
        'content-type': 'application/json',
        'accepts': 'application/json'
...
export const getNowPlaying = () => {
  return dispatch => {
    return fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.THEMOVIEDB_API_KEY}&language=en-US&page=1`)
    .then(r => r.json())
...
export const getNowPlayingTwo = () => {
  return dispatch => {
    return fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.THEMOVIEDB_API_KEY}&language=en-US&page=2`)
    .then(r => r.json())
...
export const getNowPlayingThree = () => {
  return dispatch => {
    return fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.THEMOVIEDB_API_KEY}&language=en-US&page=3`)
    .then(r => r.json())
